...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT COUNT(*) FROM category_details WHERE Category_name = ?";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("s", $categoryName);
    $checkStmt->execute();
    $checkStmt->bind_result($categoryCount);
    $checkStmt->fetch();
    $checkStmt->close();

    if ($categoryCount > 0) {
        echo "<script>alert('Error: Category with the same name already exists.');</script>";
    } else {
        $status = 1;
        $stmt = $conn->prepare("INSERT INTO category_details (Category_name, Category_description, Status) VALUES (?, ?, ?)");
        $stmt->bind_param("ssi", $categoryName, $description, $status);
...
<div class="content">
    ...
    $('form').submit(function(event) {
        ...
        if (category_name == '') {
            $('#category_name').after('<div class="alert alert-danger">Please enter a category name</div>');
            event.preventDefault();
        }
        if (category_details == '') {
            $('#category_details').after('<div class="alert alert-danger">Please enter category details</div>');
            event.preventDefault();
        }
    });
});     
